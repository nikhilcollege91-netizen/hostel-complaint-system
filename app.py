...
def init_db():
    """Create tables and default warden account if not present"""
    db = get_db()
    db.execute("""
    ...
    # ✅ Create default warden account if not exists
    warden_email = "hostelwarden.cu@gmail.com"
    warden_password = generate_password_hash("CUWARDEN")
    ...
    if not existing:
        db.execute("INSERT INTO users (name, email, password, is_warden) VALUES (?, ?, ?, 1)",
                   ("Warden", warden_email, warden_password))
        print("✅ Default Warden created -> Email: hostelwarden.cu@gmail.com | Password: CUWARDEN")
    db.commit()
